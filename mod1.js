...
$(document).ready(function(){
    ...
    $("#btngenerate").on("click",function(){
        ...
        $.ajax({
            url : '/feeds/posts/summary?alt=json-in-script&callback=recentposts&max-results=99999',
            type: 'get',
            dataType: 'jsonp',
...
$(document).ready(function(){
    ...
    gotolink = $("#gotolink"),
    timer = $('#timer');
    var x; // Declare the interval variable outside the functions for scope access
    ...
    function gotolinkcountdown(){
        ...
        x = setInterval(function() {
            ...
            gotolink.html('<span class="bi bi-clock-fill"></span> Please Wait...');

            if (distance < 0) {
                clearInterval(x);
                gotolink.prop('disabled',false);
                gotolink.html('<span class="bi bi-check-circle-fill"></span> Press here');
            }
        }, 1000);
    }				
    ...
    // Visibility change event to stop/start countdown
    document.addEventListener('visibilitychange', function() {
        if (document.hidden) {
            clearInterval(x); // Stop the countdown
        } else {
            if (request == true && $('#gotolink').is(':visible')) {
                gotolinkcountdown(); // Restart the countdown
            }
        }
    });
